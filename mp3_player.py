...
window = tk.Tk()
window.title("Tinh Dong's Music Player App")
window.geometry("725x550")
window.configure(bg='#212525')

# Create a label for the music player title
l_music_player = tk.Label(window, text="Tinh Dong's Music Player", font=("Times New Roman", 30, "bold" ), fg='#e2feff', bg='#212525')
...
btn_select_folder = ctk.CTkButton(window, text="Select Music Folder",
                                  ...
                                  font=("Times New Roman", 18), text_color='#e2feff', fg_color='#4c4f4f')
btn_select_folder.pack(pady=20)

# Create a listbox to display the available songs
lbox = tk.Listbox(window, width=80, font=("Times New Roman", 16), bg='#4c4f4f', fg='#e2feff')
lbox.pack(pady=10)

# Create a frame to hold the control buttons
btn_frame = tk.Frame(window, background='#212525')
btn_frame.pack(pady=20)

# Create a button to go to the previous song
btn_previous = ctk.CTkButton(btn_frame, text="<", command=previous_song,
                            width=50, font=("Times New Roman", 18),text_color='#e2feff', fg_color='#4c4f4f')
...
btn_play = ctk.CTkButton(btn_frame, text="Play", command=play_music, width=50,
                         font=("TkDefaultFont", 18),text_color='#e2feff', fg_color='#4c4f4f')
